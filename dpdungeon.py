def newHealth(fromH,toH):
    
    if toH > 0: return 0 if toH >= fromH  else fromH-toH
    if toH < 0: return fromH + abs(toH)
    return fromH   
...
class Solution(object):
    def calculateMinimumHP(self, dungeon):
        ...
        paths[-1] = 0 if dungeon[-1][-1] >= 0 else abs(dungeon[-1][-1])

        #initialize bottom row
        for x in row:
            paths[x] = newHealth(fromH=paths[x+1],toH=dungeon[-1][x])
        ...
        for y in reversed(range(height)):
            ...
            paths[-1] = newHealth(fromH=paths[-1],toH=dungeon[y][-1])


            #paths[:-1] = [ min(newHealth(fromH=paths[x+1],toH=dungeon[y][x]),newHealth(fromH=paths[x],toH=dungeon[y][x])) for x in row ]
            ...
            #for x in row:
                ...
                #paths[x] = min(newHealth(fromH=paths[x+1],toH=dungeon[y][x]),newHealth(fromH=paths[x],toH=dungeon[y][x]))
