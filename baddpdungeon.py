class path(object):
    def __init__(self, chealth, shealth):
        self.chealth = chealth
        self.shealth = shealth

    def __lt__(self,other):
        if self.shealth != other.shealth: return self.shealth < other.shealth
        return self.chealth > other.shealth
    def __gt__(self,other):
        if self.shealth != other.shealth: return self.shealth > other.shealth
        return self.chealth < other.shealth

    def __repr__(self):
        return "(st:%d,ch:%d)"%(self.shealth,self.chealth)
    def __str__(self):
        return self.__repr__()
    
def updatePath(dSquare,pathfrom,pathTo):
    shealth,chealth = pathfrom.shealth, pathfrom.chealth

    if dSquare < 0 and chealth <= abs(dSquare):
        pathTo.chealth = 1
        pathTo.shealth = shealth + abs(chealth + dSquare) + 1
    else:
        pathTo.chealth = chealth + dSquare
        pathTo.shealth = shealth
...
class Solution(object):
    def calculateMinimumHP(self, dungeon):
        ...
        if dungeon[0][0] > 0: 
            shealth = 1
            chealth = dungeon[0][0] + 1
        elif dungeon == 0:
            shealth, chealth = 0,0
        else:
            shealth = abs(dungeon[0][0]) + 1
            chealth = 1

        paths = [path(shealth= shealth, chealth = chealth)]

        for d in dungeon[0][1:]:
            paths.append(path(0,0))
            updatePath(d,pathfrom=paths[-2],pathTo=paths[-1])


        for line in dungeon: print line
        ...
        for y in range(1,len(dungeon)):
            ...
            updatePath(dungeon[y][0],paths[0],paths[0])

            for x in range(1,len(dungeon[y])):

                print ' ' * len(str(paths[:x])), paths[x:]
                print paths[:x], "   ", dungeon[y][x]
                raw_input()

                updatePath(dungeon[y][x], pathfrom= min(paths[x-1],paths[x]), pathTo = paths[x] )

                
        return paths[-1].shealth
